...
raoq <- FD::dbFD(as.matrix(traitmatrix), as.matrix(compmatrix))
...
cwm_roaq23 <- data.frame(leafn=leafn$RaoQ,
                         ldmc=ldmc$RaoQ,
                         srl=srl$RaoQ,
...
cwm_roaq23$trt <- factor(cwm_roaq23$trt, levels = c('ir','dt','fd','rand'),ordered = TRUE)
...
rootdiam <- FD::dbFD(as.matrix(traitmatrix[,"rootdiam", drop=F]), as.matrix(compmatrix))
#veg <- FD::dbFD(as.matrix(traitmatrix[,"veg", drop=F]), as.matrix(compmatrix))
cwm_roaq22 <- data.frame(leafn=leafn$RaoQ,
                         ldmc=ldmc$RaoQ,
                         srl=srl$RaoQ,
                         rootdiam=rootdiam$RaoQ,
                         full=raoq$RaoQ)
...
cwm_roaq22$trt <- factor(cwm_roaq22$trt, levels = c('ir','dt','fd','rand'),ordered = TRUE)
