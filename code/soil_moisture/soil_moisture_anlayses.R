...
anova(lmer(Moisture~Year*seedtrt*droughttrt+(1|Block), smdatsub))
summary(lm(Moisture~droughttrt, smdat))
ggplot(smdatsub, aes(y=Moisture, x=seedtrt, fill=droughttrt))+
  geom_boxplot()+
...
annotate_figure(
  ggarrange(smtrtplot, ggarrange(smldmcplot, smrdfdplot, 
            smfullplot, common.legend = T, nrow=1), 
